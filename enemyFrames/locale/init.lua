...
function f:GetClassToken(class)
	for eng_class, classes in pairs(LOCALIZED_CLASS) do
		if eng_class == class then return string.upper(eng_class) end
...
local LOCALIZED_RACE = {
	...
	["Undead"] = {"Нежить", "Untoter", "Mort-vivant", "亡灵", "不死族", "언데드", "No-muerto"},
}

function f:GetRaceToken(race)
	for eng_race, races in pairs(LOCALIZED_RACE) do
		if eng_race == race then
			eng_race = eng_race == 'Undead' and 'Scourge' or eng_race == 'Night Elf' and 'NightElf' or eng_race		
			return string.upper(eng_race)
		end
